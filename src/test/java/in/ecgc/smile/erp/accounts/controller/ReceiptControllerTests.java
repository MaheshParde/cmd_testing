...
//		String receipts = mapper.writeValueAsString(receipt);
//		when(receiptService.updateReceipt(100, receipt)).thenReturn(1);
//		mockMvc.perform(MockMvcRequestBuilders.put("/receipt/update/{receiptId}",100)
...
//		when(receiptService.viewByRecieptId(100)).thenReturn(receipt);
//		mockMvc.perform(MockMvcRequestBuilders.get("/receipt/view/{receiptId}", 100)
//				.contentType(MediaType.APPLICATION_STREAM_JSON))
//		.andExpect(status().isOk())
//		.andExpect(MockMvcResultMatchers.content().contentType("application/json"))
