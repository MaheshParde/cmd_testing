...
public class ExtPaymentAdviceController {
	...
	@PostMapping(value = "/add/{seqNo}/{moduleCd}")
	@ApiOperation(value = "Create new payment advice entry", response = ResponseEntity.class)
	public ResponseEntity<Integer> addPaymentAdvice(@PathVariable("seqNo") @NotBlank Integer seqNo,
			@PathVariable("moduleCd") @NotBlank Integer moduleCd
			,@Valid @RequestBody PaymentAdvice payAdvice){
		
		System.err.println("Inside accounts be extapyment advice ctrl save");
		Integer res = payAdviceService.addPaymentAdvice(payAdvice);
		System.err.println("Result after saving pay adv object in db : "+res);
		//if record inserted successfully in accounts database, 
		//then update in callers database
		
		System.err.println("Accounts BE Ctrl, SeqNo : "+seqNo+" module code : "+moduleCd);
		
		if (res == 1) {
			
			String extRes = extPaymentAdviceService.savePaymentAdvice(seqNo,moduleCd, payAdvice.getAdviceNo().toString());
			System.err.println("advice no against Ref no : "+seqNo+" is : "+extRes);
			if (extRes != null) {
				Integer extPayAdvNo = Integer.parseInt(extRes);
				return new ResponseEntity<Integer>(extPayAdvNo, HttpStatus.OK);
			}else
