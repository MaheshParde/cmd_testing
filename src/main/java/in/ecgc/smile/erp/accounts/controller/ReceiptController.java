...
public class ReceiptController {
	...
	public ResponseEntity<Integer> addReceipt(@RequestBody Receipt receipt) {
	//	System.err.println("inside add controller");
		...
		try {
			...
			if(result >=1) {
				//System.out.println("invoice number from controller if:" + receipt.getInvoiceNo());
				model = new ResponseEntity<>(result,HttpStatus.CREATED);
			}else {
			//	System.out.println("invoice number from controller else:" + receipt.getInvoiceNo());
				model = new ResponseEntity<>(0,HttpStatus.BAD_REQUEST);
			}
		} 
		catch (Exception e) {
			//System.out.println("exception in controller");
			//System.out.println("invoice number from controller catch block:" + receipt.getInvoiceNo());
			model = new ResponseEntity<>(0,HttpStatus.BAD_REQUEST);
			e.printStackTrace();
